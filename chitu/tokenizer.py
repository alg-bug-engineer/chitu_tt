...
class TokenizerHFWrapper:
    """
    Wrapper for an existing HuggingFace AutoTokenizer object.
    Used for TT-Qwen integration where tokenizer is already created.
    """
    def __init__(self, tokenizer, force_full_seq_decode: bool = False):
        """
        Initialize the wrapper with an existing tokenizer.
        
        Args:
            tokenizer: An existing AutoTokenizer instance
            force_full_seq_decode: If true, every time a new token is generated,
                the output string will be re-decoded from all the generated tokens.
        """
        self.force_full_seq_decode = force_full_seq_decode
        ...
        if hasattr(args, "models") and hasattr(args.models, "eos_token_id"):
            ...
            config_eos_tokens = tokenizer.eos_token_id
...
class ChatFormatHF:
    def __init__(self, tokenizer: Union[TokenizerHF, TokenizerHFWrapper], processor: Processor = None):
        self.tokenizer = tokenizer
