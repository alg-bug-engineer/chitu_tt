...
#
# Integrated from tt_qwen/tt/utility_functions.py
...
def enable_persistent_kernel_cache():
    ...
    """
    if not has_ttnn:
        raise ImportError("ttnn is required for enable_persistent_kernel_cache")
    logger.warning(
...
def disable_persistent_kernel_cache():
    ...
    """
    if not has_ttnn:
        raise ImportError("ttnn is required for disable_persistent_kernel_cache")
...
def enable_memory_reports():
    ...
    """
    if not has_ttnn:
        raise ImportError("ttnn is required for enable_memory_reports")
...
def disable_memory_reports():
    ...
    """
    if not has_ttnn:
        raise ImportError("ttnn is required for disable_memory_reports")
...
def torch2tt_tensor(
    ...
    tt_device,
    tt_layout=ttnn.TILE_LAYOUT if has_ttnn else None,
    tt_memory_config=None,
    tt_dtype=ttnn.bfloat16 if has_ttnn else None,
):
    if not has_ttnn:
        raise ImportError("ttnn is required for torch2tt_tensor")
    if tt_memory_config is None:
...
def tt_tensors_to_torch_tensors(
    tt_tensors_device, mesh_device: Union[ttnn.MeshDevice, ttnn.Device], concat_dim: int = 0
):
    if not has_ttnn:
        raise ImportError("ttnn is required for tt_tensors_to_torch_tensors")
...
def tt2torch_tensor(tt_tensor):
    if not has_ttnn:
        raise ImportError("ttnn is required for tt2torch_tensor")
...
def tt_to_torch_tensor(tt_tensor):
    if not has_ttnn:
        raise ImportError("ttnn is required for tt_to_torch_tensor")
...
def torch_to_tt_tensor_rm(py_tensor, device, shape=None, put_on_device=True):
    if not has_ttnn:
        raise ImportError("ttnn is required for torch_to_tt_tensor_rm")
    if shape is None:
...
def torch_to_tt_tensor(py_tensor, device):
    if not has_ttnn:
        raise ImportError("ttnn is required for torch_to_tt_tensor")
    shape = list(py_tensor.size())
    while len(shape) < 4:
        shape.insert(0, 1)
...
def pad_by_zero(
    ...
    if not has_ttnn:
        raise ImportError("ttnn is required for pad_by_zero")
    if tt_memory_config is None:
        tt_memory_config = ttnn.MemoryConfig(ttnn.TensorMemoryLayout.INTERLEAVED)
...
def unpad_from_zero(x, desired_shape):
    if not has_ttnn:
        raise ImportError("ttnn is required for unpad_from_zero")
...
def ulp(x: Union[ttnn.Tensor, torch.Tensor]) -> Union[ttnn.Tensor, torch.Tensor]:
    ...
    if has_ttnn and isinstance(x, ttnn.Tensor):
        x = ttnn.to_torch(x)
    ...
    if received_ttnn_input and has_ttnn:  # Ensures that type(input) == type(output)
...
def ttP(x, count=4, offset=0, stride=1):
    if type(x) == torch.Tensor:
        ...
        if not has_ttnn:
            raise ImportError("ttnn is required for ttP")
        tt_out = x.cpu()
...
def is_e75(device):
    if not has_ttnn:
        raise ImportError("ttnn is required for is_e75")
...
def is_x2_harvested(device):
    if not has_ttnn:
        raise ImportError("ttnn is required for is_x2_harvested")
...
def is_single_chip():
    if not has_ttnn:
        raise ImportError("ttnn is required for is_single_chip")
...
def is_grayskull():
    if not has_ttnn:
        raise ImportError("ttnn is required for is_grayskull")
    ARCH_NAME = ttnn.get_arch_name()
...
def ttl_complex_2_torch_complex(tt_tensor):
    if not has_ttnn:
        raise ImportError("ttnn is required for ttl_complex_2_torch_complex")
