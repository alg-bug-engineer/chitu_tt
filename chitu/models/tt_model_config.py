...
if has_ttnn:
    # Note: llama31_81b_model is a reference implementation, only used in testing/validation
    # We'll make it optional to avoid dependency issues
    try:
        import sys
        import os
        project_root = os.path.abspath(os.path.join(os.path.dirname(__file__), "..", ".."))
        tt_qwen_path = os.path.join(project_root, "tt_qwen")
        if tt_qwen_path not in sys.path:
            sys.path.insert(0, tt_qwen_path)
        from tt.llama31_81b_model import Transformer
    except ImportError:
    ...
    from chitu.models.tt_load_checkpoints import (
    convert_hf_to_meta,
    convert_meta_to_hf,
...
class ModelArgs:
        ...
        if self.dummy_weights:
            if self.checkpoint_type == CheckpointType.HuggingFace:
                ...
                if Transformer is None:
                    raise ImportError("llama31_81b_model is required for dummy weights with Meta checkpoint type. This is a reference implementation.")
    ...
    def reference_lm_head(self):
        if self.checkpoint_type == CheckpointType.Meta:
            from llama31_8b_model import ColumnParallelLinear
    ...
    def reference_transformer(self, wrap=True, load_checkpoint=False):
        if self.checkpoint_type == CheckpointType.Meta:
            if Transformer is None:
                raise ImportError("llama31_81b_model is required for reference transformer with Meta checkpoint type. This is a reference implementation.")
    ...
    def reference_rms_norm(self):
        if self.checkpoint_type == CheckpointType.Meta:
            from llama31_8b_model import RMSNorm
    ...
    def reference_mlp(self):
        if self.checkpoint_type == CheckpointType.Meta:
            from llama31_8b_model import FeedForward
    ...
    def reference_decoder(self):
        if self.checkpoint_type == CheckpointType.Meta:
            from llama31_8b_model import TransformerBlock
    ...
    def reference_attention(self):
        if self.checkpoint_type == CheckpointType.Meta:
            from llama31_8b_model import Attention
...
class DecodersPrecision:
    ...
    def get_tensor_dtype(self, decoder_id, tensor: TensorGroup):
        ...
        tensor_dtype_settings = self.decoder_optimizations[decoder_id].tensor_dtype_settings
        # If tensor type is not in settings, use default (None means use original dtype)
        precision_setting = tensor_dtype_settings.get(tensor, None)
        return precision_setting_lookup[precision_setting]
